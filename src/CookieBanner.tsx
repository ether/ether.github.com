...
export const CookieBanner = ()=>{
    ...
    return createPortal(
        <div className={`sticky bottom-0 bg-gray-800 p-5 text-white grid grid-cols-[1fr_auto]
         ${cookiesAccepted !==undefined ?'opacity-0 pointer-events-none h-0 overflow-hidden p-0': 'opacity-100'}`}>
            This page uses Google analytics to track page visits.
            Are you okay with that?
            <div className="grid gap-3 grid-cols-2">
                <button className="border-2 rounded" onClick={()=>setCookiesAccepted(false)}>Decline</button>
                <button className="rounded bg-blue-600 p-2" onClick={()=>setCookiesAccepted(true)}>Accept cookies</button>
            </div>
        </div>
